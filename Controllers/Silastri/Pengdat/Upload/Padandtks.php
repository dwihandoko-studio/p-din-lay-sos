...
use App\Models\Silastri\Pengdat\Upload\PadandtksModel;
use Config\Services;
...
class Padandtks extends BaseController
{
    ...
    {
        return redirect()->to(base_url('silastri/pengdat/upload/padandtks/data'));
    }

    public function data()
    {
        $data['title'] = 'Upload Data Padan DTKS';
        ...
        return view('silastri/pengdat/upload/padandtks/index', $data);
    }
    ...
    public function upload()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $response->data = view('silastri/pengdat/upload/padandtks/upload');
            return json_encode($response);
        }
    }
    ...
    public function uploadSave()
    {
        ...
        $existingniks = $this->_db->table('ref_padandtks')->select('nik')->get()->getResultArray();
        ...
        try {
            ...
            for ($row = 3; $row <= $highestRow; $row++) {
                ...
                $nik = isset($rowData[$row]['A']) ? trim($rowData[$row]['A']) : null;
                ...
                $data = [
                    'nik' => $nik,
                    'nama_input' => $rowData[$row]['B'],
                    'nama_padan_dtks' => $rowData[$row]['C'],
                    'status_dtks' => $rowData[$row]['D'],
                    'bansos' => $rowData[$row]['E'],
                    'kabupaten' => $rowData[$row]['F'],
                ];
                ...
                if ($row % 1000 === 0) {
                    if (!empty($dataToInsert)) {
                        ...
                        $this->_insertIgnoreBatch('ref_padandtks', $dataToInsert);
                    }
                    // $this->_db->table('ref_dtks')->insertBatch($dataToInsert);
                    if (!empty($dataToUpdate)) {
                        $this->_db->table('ref_padandtks')->updateBatch($dataToUpdate, 'nik');
                    }
                    $dataToInsert = [];
                    $dataToUpdate = [];
                }
        ...
        // Handle file upload
        $dir = FCPATH . "upload/matching-padandtks";
        ...
        $this->_db->table('tb_matching_padandtks')->insert($data);
    ...
    public function delete()
    {
        ...
        if (!$this->validate($rules)) {
            ...
            $current = $this->_db->table('tb_matching_padandtks')
                ->where('id', $id)
                ->get()->getRowObject();
            ...
            if ($current) {
                ...
                $this->_db->transBegin();
                try {
                    $this->_db->table('tb_matching_padandtks')->where('id', $current->id)->delete();
                } catch (\Throwable $th) {
                    $this->_db->transRollback();
                    $response = new \stdClass;
                    $response->status = 400;
                ...
                if ($this->_db->affectedRows() > 0) {
                    ...
                    try {
                        ...
                        unlink(FCPATH . "upload/matching-padandtks/$file");
                    } catch (\Throwable $th) {
                        //throw $th;
                    }
                    $response = new \stdClass;
