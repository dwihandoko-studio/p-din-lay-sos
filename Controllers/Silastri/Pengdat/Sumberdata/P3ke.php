...
use App\Models\Silastri\Pengdat\Sumberdata\P3keModel;
use Config\Services;
...
class P3ke extends BaseController
{
    ...
    public function getAll()
    {
        ...
        foreach ($lists as $list) {
            ...
            $row[] = $list->tgl_lahir;
            $row[] = $list->pekerjaan;
            $row[] = $list->status_pekerjaan;
            $row[] = $list->pendidikan;
            $row[] = $list->status_kawin;
            $row[] = $list->usia;
    ...
    {
        return redirect()->to(base_url('silastri/pengdat/sumberdata/p3ke/data'));
    }

    public function data()
    {
        $data['title'] = 'SUMBER DATA P3KE';
        $Profilelib = new Profilelib();
        $user = $Profilelib->user();
        if ($user->status != 200) {
            session()->destroy();
        ...
        return view('silastri/pengdat/sumberdata/p3ke/index', $data);
    }
