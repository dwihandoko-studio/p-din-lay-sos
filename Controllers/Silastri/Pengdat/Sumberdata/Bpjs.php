...
class Bpjs extends BaseController
{
    ...
    public function getAll()
    {
        ...
        foreach ($lists as $list) {
            ...
            $row[] = $list->pekerjaan;
            $row[] = $list->pendidikan;
            $row[] = $list->prioritas_verval;
            $row[] = $list->padan_siak;
            $row[] = $list->kepersertaan_bpjs;
            $row[] = $list->status_bpjs;
            $row[] = $list->hubungan_keluarga;
            $row[] = $list->usia;
            $row[] = $list->kelompok_usia;
            $row[] = $list->kecamatan;
            $row[] = $list->kelurahan;
            $row[] = $list->kode_kemdagri;
    ...
    {
        return redirect()->to(base_url('silastri/pengdat/sumberdata/bpjs/data'));
    }

    public function data()
    {
        $data['title'] = 'SUMBER DATA BPJS';
        $Profilelib = new Profilelib();
        $user = $Profilelib->user();
        if ($user->status != 200) {
            session()->destroy();
        ...
        return view('silastri/pengdat/sumberdata/bpjs/index', $data);
    }
