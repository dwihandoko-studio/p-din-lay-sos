...
class Bltdd extends BaseController
{
    ...
    public function getAll()
    {
        ...
        foreach ($lists as $list) {
            ...
            $row[] = $list->jumlah_anggota_keluarga;
            $row[] = $list->alamat;
            $row[] = $list->rt;
            $row[] = $list->rw;
            $row[] = $list->jenis_pekerjaan;
            $row[] = $list->kriteria_blt;
    ...
    {
        return redirect()->to(base_url('silastri/pengdat/sumberdata/bltdd/data'));
    }

    public function data()
    {
        $data['title'] = 'SUMBER DATA BLTDD';
        $Profilelib = new Profilelib();
        $user = $Profilelib->user();
        if ($user->status != 200) {
            session()->destroy();
        ...
        return view('silastri/pengdat/sumberdata/bltdd/index', $data);
    }
