...
use App\Models\Silastri\Su\Laporan\P3keModel;
use Config\Services;
...
class P3ke extends BaseController
{
    ...
    public function getAll()
    {
        ...
        foreach ($lists as $list) {
            ...
            //             </div>
            //         </div>';
            $action = '<a href="javascript:actionDetail(\'' . $list->id_individu . '\', \'' . $list->nik . '\', \'' . str_replace('&#039;', "`", str_replace("'", "`", $list->nama)) . '\');"><button type="button" class="btn btn-primary btn-sm btn-rounded waves-effect waves-light mr-2 mb-1">
            ...
            $row[] = $list->usia;
            $row[] = $list->terdaftar_dtks;
            $row[] = $list->penerima_bpnt;
            $row[] = $list->penerima_bst;
            $row[] = $list->penerima_pkh;
            $row[] = $list->penerima_sembako;
            $row[] = $list->penerima_prakerja;
            $row[] = $list->penerima_kur;
            $row[] = $list->penerima_cbp;
            $row[] = $list->padan_penerima_bpnt;
            $row[] = $list->padan_penerima_pkh;
            $row[] = $list->padan_penerima_pbi;
            $row[] = $list->padan_penerima_blt;
    ...
    {
        return redirect()->to(base_url('silastri/su/laporan/p3ke/data'));
    }

    public function data()
    {
        $data['title'] = 'Data Laporan P3KE';
        $Profilelib = new Profilelib();
        $user = $Profilelib->user();
        if ($user->status != 200) {
            session()->destroy();
        ...
        return view('silastri/su/laporan/p3ke/index', $data);
    }
